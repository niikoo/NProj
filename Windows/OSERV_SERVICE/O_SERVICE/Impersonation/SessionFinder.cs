    ...
    class SessionFinder
    {
        ...
        public IntPtr GetSessionByUser(string domain, string userName)
        {
            ...
            try
            {
                ...
                {
                    IntPtr pCurrentSession = new IntPtr(pSessionsBuffer.ToInt64() + Marshal.SizeOf(typeof(WTS_SESSION_INFO)) * i);
                    WTS_SESSION_INFO sessionInfo = (WTS_SESSION_INFO)Marshal.PtrToStructure(pCurrentSession,typeof(WTS_SESSION_INFO));

                    if (string.Compare(GetUserName(sessionInfo.SessionId), userName, true) == 0 && string.Compare(GetDomain(sessionInfo.SessionId), domain,true) == 0)
                    {
